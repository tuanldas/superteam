...
> Status: DRAFT v3 | Created: 2026-03-23 | Revised: 2026-03-24 (registry → gather+classify redesign)
...
9. **Gather + classify over hardcoded table** — detector gathers raw data, Claude classifies. Zero maintenance for new frameworks. Replaces the original hardcoded detection signals table.
...
13. **Mobile as first-class type** — distinct principles for platform targets, device testing, native bridges. Not a subset of frontend.
14. **Raw data in context block** — manifests, config files, directory structure visible to commands and user. Classification is verifiable, not opaque.
15. **Classification confidence guidelines** — signal-based rules for assigning confidence scores. Prevents arbitrary numbers while keeping numeric precision.
...
16. FastAPI project (pyproject.toml, no manage.py) — classified as backend with reasonable confidence?
17. Flutter project (pubspec.yaml with flutter dep) — classified as mobile, not frontend?
18. Project with only `pyproject.toml` and unknown deps — confidence < 0.5, asks user?
19. Deno project (`deno.json`) — ecosystem detected, frameworks classified from deps?
20. .NET project (`*.csproj`) — classified as backend?
21. Multiple manifests (package.json + pyproject.toml, no workspace) — confidence < 0.5, asks user?
22. Mobile project — `/st:plan` includes platform target verification?
