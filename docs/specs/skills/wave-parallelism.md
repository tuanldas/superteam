...
1. **File-ownership as absolute rule** — GSD's primary safety mechanism, proven effective. Claude WILL rationalize "different functions in same file" — must be explicitly blocked.
2. **Lean orchestrator pattern** — Pass paths not content. Each agent has 200k context. Loading content in orchestrator wastes tokens and risks context overflow.
3. **`--no-verify` + post-wave hooks** — GSD's solution to pre-commit hook lock contention. Hooks still run, just once per wave.
4. **Spot-check verification** — Don't trust Task() completion signals alone. Verify filesystem + git state. GSD learned this from unreliable returns.
5. **Wave structure display before execution** — User sees and approves the wave plan. Prevents "why did you parallelize these?" after the fact.
6. **Node repair escalation ladder** — RETRY → DECOMPOSE → PRUNE → ESCALATE. Each level more expensive. Budget prevents infinite retry loops.
7. **Vertical slices over horizontal layers** — Maximizes Wave 1 parallelism. Horizontal layers create artificial sequential chains.
8. **Inter-wave dependency validation** — GSD's "Cross-Plan Wiring Gap" detection. Prevents Wave 2 agents from failing because Wave 1 output is missing or wrong.
9. **Fallback path explicit** — Not all environments support reliable parallel execution. Detect early, fall back gracefully.
10. **No separate reference file** — All methodology fits in ~380 lines. Wave protocol is one coherent concept, splitting would fragment understanding.
11. **Agent limits by type** — Code agents heavier than research agents. 3-4 code, 5 research. Nuance vs flat cap.
12. **Fresh agent, never resume** — GSD's explicit finding: "Resume relies on internal serialization that breaks with parallel tool calls."
13. **Implicit dependencies** — File-ownership misses DB state, env vars, ports, API rate limits. Explicit table of shared resources.
14. **Cascade handling** — PRUNE cascades to dependent tasks. Display impact, user approves. >50% cascade = restructure plan.
15. **Post-wave test scope** — Wave-owned tests + integration tests at boundaries. Full suite only on last wave. git bisect for failure isolation.
16. **Agent skills injection** — Agents receive applicable skills (TDD, debugging) in context. Without this, agents skip methodology.

## Testing Plan

1. Two tasks modifying same file — does Claude put them in different waves or same?
2. Three independent tasks — does Claude assign all to Wave 1?
3. Task B depends on Task A's output — does Claude put B in Wave 2?
4. Wave completes but one agent's completion signal lost — does Claude spot-check or block?
5. Wave 1 completes, Wave 2 task depends on file that wasn't created — does Claude detect the gap?
6. config.parallelization is false — does Claude fall back to sequential?
7. Task fails in Wave 1 — does Claude apply RETRY before DECOMPOSE?
8. 8 independent tasks — does Claude cap agents at 5 and split into sub-waves?
9. Pre-commit hooks fail after wave — does Claude report before proceeding to next wave?
10. Claude rationalizes "they edit different functions in same file" — does it still enforce file-ownership rule?
11. Agent modifies file outside its ownership list — does it stop or continue?
12. Monorepo with workspace-scoped tasks — does Claude use project-awareness scope for dependency analysis?
13. Two tasks share DB state (seed data + run tests) — does Claude detect implicit dependency?
14. Task 2 PRUNED, Task 4 depends on Task 2 — does Claude cascade prune Task 4?
15. Task 2 PRUNED, Task 5 depends on Task 2 + Task 3 — does Claude determine Task 5 can continue?
16. Post-wave tests fail — does Claude identify which task's commit caused it?
17. 5 code-execution agents vs 5 research agents — does Claude apply different limits?
